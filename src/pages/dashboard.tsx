...
import { getRelationships } from 'apiSdk/relationships';
import { RelationshipInterface } from 'interfaces/relationship';
import { getCommunicationLogs } from 'apiSdk/communication-logs';
import { CommunicationLogInterface } from 'interfaces/communication-log';
import { getMeetingNotes } from 'apiSdk/meeting-notes';
import { MeetingNoteInterface } from 'interfaces/meeting-note';

import { RelationshipListPage as ShowCaseList } from 'pages/relationships';

function DashboardPage() {
  const { session } = useSession();

  const relationshipStat = useSWR<PaginatedInterface<RelationshipInterface>>(
    () => `/relationships`,
    () => getRelationships({ limit: 1, offset: 0 }),
  );

  const communicationLogStat = useSWR<PaginatedInterface<CommunicationLogInterface>>(
    () => `/communication-logs`,
    () => getCommunicationLogs({ limit: 1, offset: 0 }),
  );

  const meetingNoteStat = useSWR<PaginatedInterface<MeetingNoteInterface>>(
    () => `/meeting-notes`,
    () => getMeetingNotes({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Relationships',
      result: relationshipStat?.data?.totalCount,
      isLoading: relationshipStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Communication Logs',
      result: communicationLogStat?.data?.totalCount,
      isLoading: communicationLogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Meeting Notes',
      result: meetingNoteStat?.data?.totalCount,
      isLoading: meetingNoteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`relationships`} passHref legacyBehavior>
                  <Button
